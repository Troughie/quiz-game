        ...
        (set) => ({
            ...
            editSlide: (type: UpdateAction, slideIndex?: number | string) =>
                set((state) => {
                    ...
                    const isSelectedSlide =
                        slideIndex === state.selectedSlide?._id ||
                        slideIndex === state.selectedSlide?.index;

                    let objectReturn = {
                        ...state,
                        slides: updatedSlides,
                    };
                    if (isSelectedSlide && state.selectedSlide) {
                        const updatedSlide = applySlideAction(
                            state.selectedSlide,
                            type
                        );
                        objectReturn = {
            ...
            removeSlide: (slideId: string) =>
                set((state) => {
                    ...
                    const updateSlide = Slides.filter(
                        (sl) => sl._id !== slideId
                    );
