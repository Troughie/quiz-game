...
export const useQuizFunction = () => {
  ...
  const { mutate: createSlideServer } = useRequest({
    ...
    onSuccess: (data: ResponseBase<Question>) => {
      ...
      editSlide({ type: "UPDATE", updates: slideEdited }, slideEdited.index);
      markAsSaved();
      restoreValueFunc();
    },
    onError: (err) => {
  ...
  const { mutate: editSlideServer } = useRequest({
    mutationFn: editSlideAPI,
    onSuccess: (data: ResponseBase<Question>) => {
      markAsSaved();
      const slideEdited = data.data;

      editSlide({ type: "UPDATE", updates: slideEdited }, slideEdited._id);
      restoreValueFunc();
  ...
  const handleEditQuizApi = async (value: Partial<Quiz>) => {
    if (!quiz._id) {
      console.log("Creating quiz before editing slide...");
  ...
  const handleEditSlide = async (
    ...
    if (!slide._id) {
      ...
      createSlideServer(slide);
    } else {
      console.log("Edit existing slide...");
      editSlideServer(slide);
    }
  };
    ...
    switch (typeQuiz) {
      case "checkBoxSlide":
        newAnswers[index] = {
          ...
          text: value,
          isCorrect:
            isCorrect !== undefined ? isCorrect : newAnswers[index].isCorrect,
        };
        break;
  ...
  const handleInputChange = ({
    ...
    setInput((prev) => {
      ...
      if (mode === "answers" && index !== undefined && selectedSlide?.answers) {
