...
const AnswerIndicator = ({
  ...
  const getClassName = () =>
    cn(
      ...
      isAnswerCorrect ? "bg-green-500 rounded-full" : "bg-red-500 rounded-lg"
    );
  ...
  return (
    <m.div
      animate={controls}
      initial={{ scale: 0 }}
      transition={{ type: "spring", duration: ANIMATION_CONFIG.springDuration }}
      className={
        isCheckbox
          ? getClassName()
    ...
    >
      {isCheckbox ? getIcon() : <CheckIcon className="h-4 w-4 text-white" />}
    </m.div>
  );
};
