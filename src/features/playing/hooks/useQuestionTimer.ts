...
> & { getSyncedTime: () => number }) => {
  ...
  const calculateRemainingTime = useCallback((): number => {
    if (!startAt || !duration || !startCountdown) {
      return duration;
    }
      ...
      if (!skipStartTimeRef.current) {
        ...
        effectiveRemaining =
          remainingTime !== undefined
            ? remainingTime
            : Math.max(0, duration - (syncedNow - startAt));
      } else {
        ...
        const remainingAtSkipStart =
          remainingTime !== undefined
            ? remainingTime
            : Math.max(0, duration - (skipStartTimeRef.current - startAt));
  ...
  const recordTimeClick = useCallback((): number => {
    if (!startAt) return duration;

    const currentState = calculateTimerState();

    setTimerState((prev) => ({
      ...prev,
      timeClick: currentState.timeLeft,
      score: currentState.score,
    }));

    return currentState.timeLeft;
  }, [startAt, duration, calculateTimerState]);
